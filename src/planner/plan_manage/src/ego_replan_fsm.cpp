...
namespace ego_planner
{
  ...
  void EGOReplanFSM::odometryCallback(const std::shared_ptr<const nav_msgs::msg::Odometry> &msg)
  {
    ...
    // LIO odometry 的 twist.linear 是 body 坐标系（FLU），需旋转到 world 坐标系
    Eigen::Vector3d body_vel(msg->twist.twist.linear.x, msg->twist.twist.linear.y, msg->twist.twist.linear.z);
    ...
    odom_vel_ = odom_orient_ * body_vel;  // body → world
  ...
  void EGOReplanFSM::checkCollisionCallback()
  {
    ...
    // 优先检查最终目标点是否在膨胀区内，若目标本身不可达则立即急停
    bool end_occ = map->getInflateOccupancy(end_pt_);
    if (end_occ)
    {
      RCLCPP_WARN(node_->get_logger(), "最终目标点(%.2f,%.2f,%.2f)在膨胀区内，EMERGENCY_STOP",
          end_pt_(0), end_pt_(1), end_pt_(2));
