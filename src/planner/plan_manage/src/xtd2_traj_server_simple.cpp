...
// Calculate yaw angle and yaw rate
std::pair<double, double> calculate_yaw(double t_cur, Eigen::Vector3d &pos, rclcpp::Time &time_now, rclcpp::Time &time_last)
{
  ...
  // Calculate lookahead direction vector
  ...
  // Handle angle wrapping and smoothing
...
void cmdCallback()
{
  ...
  // Convert yaw to quaternion in ENU coordinate system
  Eigen::Quaterniond q_enu = Eigen::Quaterniond(Eigen::AngleAxisd(yaw_yawdot.first, Eigen::Vector3d::UnitZ()));

  // Set raw Gazebo coordinate system position and orientation
  raw_cmd.header.stamp = time_now;
  raw_cmd.header.frame_id = "gazebo_world";
